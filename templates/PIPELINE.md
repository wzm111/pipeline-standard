...
把命令按执行层标注；**每批快速检查**只放低延迟、能定位本批问题的命令，**终验/全量**只在最后一批或终验运行一次，避免每批重复跑全套。
...
> 约定:只在进入对应执行层时自检本节引用的外部 CLI；Claude Code 缺失时可按契约自动全局安装，Codex 会先请求授权。装失败则跳过对应项并在测试报告注明，不阻断流水线。
...
- 如果选择**自动提交**:
  ...
  - 提交后动作: ❏（如更新追踪矩阵、飞书通知）
- **默认人工执行**:fast/standard 由调度员输出精简变更摘要、Conventional Commits 建议与人工待办；thorough 由发布工程师(E)产出 release-notes.md。任何档位都禁止 git 写操作与部署动作。
- 人工提交后的收尾动作:❏(如更新追踪矩阵)
...
- 默认档位: ❏ standard（推荐） / ❏ fast（仅低风险直通） / ❏ thorough（里程碑全流程）
- 闸口 1: ❏ 每次人工确认 / ❏ 低风险自动进入开发（仅当无新依赖、禁区、公开接口、权限/支付/安全或范围不确定时生效；可由用户 `--auto` 单次覆盖）
- fast 的适用范围: ❏ 例如「仅既有页面文案、样式微调、明确单元修复」；超出即回 standard。
- 终验/全量命令: ❏ 列出必须在收口运行一次的命令；未声明时以 ③ 节标注为准。
- 本 run 预算(可选): ❏ 墙钟时长 / 批次数 / token 预算；约 80% 时暂停请示。
