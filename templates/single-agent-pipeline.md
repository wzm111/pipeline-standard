...
fast 只执行变更卡、定向自测和不超过 8 行的完成摘要，不写 plan/qa/release/retro。standard/thorough 的无批次计划视为单批，每一批次:
...
正文再写:每批打回轮次、CONCERNS 次数与裁决结果、自检缺装项、一行经验。只展示本档实际生成的摘要/artifact。**你不执行任何 git 写操作与部署**;git 提交、追踪矩阵打勾、部署由用户手工执行——流水线到此结束。
