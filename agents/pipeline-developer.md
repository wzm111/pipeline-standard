...
6. 每完成一项任务记录在本地工作摘要；批次收口时一次性把对应计划文件中的 `- [ ]` 改为 `- [x]`，避免为 checkbox 反复写盘。
7. 交付前自测:只跑 packet/计划标注的本批快速检查和受影响的定向测试；终验/全量命令留给最后一批或明确指定的收口阶段，不能每批重复跑。
...
批次模式(计划按批次组织时):
- 按调度员指定的批次开发;批次内任务全勾 + 本批快速检查通过 = 该批交付,向调度员简报。
- 交付后不停,继续下一批——qa 测上一批与你开下一批是并行的。**qa 测试进行中不改已交付批次的文件**,避免污染测试现场;需要动时先等该批 gate 出结果。
- 收到 qa 对某已交付批次的打回 → 暂停手头批次,优先修复该批；修完后向 qa 发送运行时对应的续跑消息请求复测，并把修复简报发给调度员。
...
team 模式:修完后向 qa 发送续跑消息告知复测(附改了什么、怎么验),并把修复简报发给调度员。
